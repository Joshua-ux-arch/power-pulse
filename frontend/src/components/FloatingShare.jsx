...
export default function FloatingShare({ area, status }) {
  if (!area || !status) return null
  return (
    <button
      onClick={() => shareOnWhatsApp(area, status)}
      className="fixed bottom-6 right-4 z-50 flex items-center gap-2 px-4 py-3 rounded-2xl text-sm font-semibold transition-all active:scale-95"
      style={{
        background: '#25d366',
        color: '#fff',
